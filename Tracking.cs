...
namespace Aftership{
	...
	public class Tracking
	{
		...
		public String getJSONPost(){
			...
			trackingJSON["tracking_number"]= _trackingNumber;
			if (_slug != null) trackingJSON["slug"]= _slug;
			if (_title != null) trackingJSON["title"]= _title;
			if (_emails != null) {
			...
			if (_customerName != null) trackingJSON["customer_name"]= _customerName;
			if (_destinationCountryISO3 != 0)
				trackingJSON["destination_country_iso3"]= _destinationCountryISO3.ToString();
			if (_orderID != null) trackingJSON["order_id"] = _orderID;
			if (_orderIDPath != null) trackingJSON["order_id_path"] = _orderIDPath;

			if (_trackingAccountNumber != null) trackingJSON["tracking_account_number"] = _trackingAccountNumber;
			if (_trackingPostalCode != null) trackingJSON["tracking_postal_code"] = trackingPostalCode;
			if (_trackingShipDate != null) trackingJSON["tracking_ship_date"] = trackingShipDate;

			if (_customFields != null) {
				JObject customFieldsJSON = new JObject();
				foreach (KeyValuePair<String, String> pair in _customFields) {
					customFieldsJSON [pair.Key] = pair.Value;
				}
				string customJSON = JsonConvert.SerializeObject(_customFields, Formatting.Indented);
